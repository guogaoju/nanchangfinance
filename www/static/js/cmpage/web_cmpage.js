...
/*
pageSaveByAct('Appr','/cmpage/page/edit?modulename=Appr&amp;procID=20&amp;actID=150&amp;status=1311&amp;linkID=3&amp;linkType=t_scf_supplierinfo&amp;linkModel=demo/scfcorecoinfo_appr&amp;listIds=','close','demo/scfcorecoinfo_appr');
 */
...
/* cmpage/page/list -- delete record */
function periodCalc(period,stock) {
    BJUI.alertmsg("confirm", "是否确定要核算当前库存以及进价？", {
        okCall: function () {
            BJUI.ajax('doajax', {
                url: "/docu/docu/periodCalc?period=" + period + "&stock=" + stock,
                loadingmask: true,
                okCallback: function (json, options) {
                    BJUI.alertmsg(json.statusCode == "200" ? "info" : "error", json.message);
                    //$('#btnSearch' + modulename).click();
