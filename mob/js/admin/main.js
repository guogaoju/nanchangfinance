...
function initview(menulist, buttonlist) {
	...
	console.log(JSON.stringify(buttonlist));
	...
	for (var i = 0; i < menulist.length; i++) {
		...
		for (var j = 0; j < menulist[i].modules.length; j++) {
			menuhtml += '<li class="mui-table-view-cell"><a id="' + menulist[i].modules[j].modulename + z + '" modulename="' +
				menulist[i].modules[j].modulename + '" parmsUrl="' + menulist[i].modules[j].parmsUrl + '" title="' + menulist[i].modules[j].title + '" url="' + menulist[i].modules[j].url + '" class="mui-navigate-right erp-menu-item">' + menulist[i].modules[j].title + '</a></li>';
			listhtml += '<li class="mui-table-view-cell mui-media mui-col-xs-4 mui-col-sm-4"><a id="' + menulist[i].modules[j].modulename + z + '" modulename="' + menulist[i].modules[j].modulename + '" parmsUrl="' + menulist[i].modules[j].parmsUrl + '" title="' + menulist[i].modules[j].title + '" url="' + menulist[i].modules[j].url + '" class="mui-navigate-right erp-menu-item"><span class="mui-icon mui-icon-chatboxes mui-active"></span></a><div class="mui-media-body">'+ menulist[i].modules[j].title +'</div></li>';
...
function switchview(title, newview, modulename, parmsUrl, url) {
	...
	if (childlist.indexOf(newview) == -1) {
		...
		console.log(newview);
		if(newview == "Home0"){
			var sub = plus.webview.create(url, newview, home_style, parm);
		}else{
