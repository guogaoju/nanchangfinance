...
module.exports = class extends CMPage {
    ...
    async updateStatus(id, actID, status, isSelf) {
        ...
        if(actID==220 && status==1570){
            let model = cmpage.service('cmpage/base');

            let businessArr = await model.query(`select * from t_scf_build_contract where id=${id}`);

            await model.query(`update t_scf_build_business set c_act=206, c_status=1420 where c_no='${businessArr[0].c_no}'`);
        }
