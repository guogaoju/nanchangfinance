...
module.exports = class extends Base {
     ...
     * @param {int} id  参数ID
     */
    async getCodeDescById(id) {
        let codes = await this.getCodes();
        for (let codeMd of codes) {
            if (codeMd.id == id) {
                console.log(codeMd.c_desc);
                return codeMd.c_desc;
            }
        }
        return 0;
    }
    ...
    async getSupplierName() {
        return await think.cache("supplierName", () => {
            return this.getTreeList(7);
        });
    }
    ...
    async getExpireColorById(day) {
        ...
        if (day<0) {
            let show = day*-1;
            return `<label style="color:red;">${show}</label>`;
        } else if(day<7) {
            return `<label style="color:red;">${day}</label>`;
        }

        else {
            return `<label style="color:green;">${day}</label>`;
        }
    }
