...
module.exports = class extends CMPage {
    ...
    async pageSave(parms) {
        ...
        if(!parms.c_path){
            parms.c_path = JSON.parse(parms.old_record).c_path;
        }
    ...
    async mobHtmlGetList() {
        ...
        for (let row of this.list.data) {
            ...
            //
           ;
            html.push( `<h5 style="color:black;font-weight:bold;">${row.c_name}</h5><p>更新时间：${row.c_time} </p><p style="color:##005094;">备注：${row.c_memo}</p><hr>`);
