...
module.exports = class extends Base {
    ...
    async getDataList() {
        ...
        // console.log(this.mod.c_datasource);
    ...
    async pageEditInit() {
        ...
        for (let edit of this.modEdits) {
            ...
            if(think.isEmpty(this.mod.parmsUrl))
                continue;
            //URL参数赋值
    ...
    async pageSave(parms) {
        ...
        if (this.mod.editID == 0) {
            ...
            if(think.isEmpty(parms[this.pk])){
                await this.model(this.mod.c_table).where(`${this.pk}=${parms['id']}`).update(this.rec);
            }else{
                await this.model(this.mod.c_table).where(`${this.pk}=${parms[this.pk]}`).update(this.rec);
            }
    ...
    async updateStatus(id, actID, status, isSelf) {
        ...
        let procID = await cmpage.service('flow/act').getProcIdByAct(actID);
        let actArr = await cmpage.service('flow/act_path').getToActPaths(actID, procID);
        ...
        let msgModel = cmpage.service('flow/msg');
        if (id > 0 && actID > 0 && status > 0) {
            ...
            for (let act of actArr){
                let assign = await cmpage.service('flow/act_assign').getAssignsByActId(act.c_to);

                for(let md of assign){
                    console.log("-----------------");
                    console.log(md);
                    console.log(md.c_act);
                    if(md.c_type_exe == 2){
                        let c_userid = md.c_link;
                        if(md.c_type==6){
                            let busArr = await this.model(this.mod.c_table).where(`${this.pk}=${id}`).find();
                            c_userid = busArr.c_user;
                        }
                        //async addMsgByType(c_modulename,c_type,c_way,c_link)
                        await msgModel.addMsgByType(this.mod.c_modulename,md.c_type,md.c_way,c_userid,id);
                    }
                }
            }
