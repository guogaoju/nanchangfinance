...
module.exports = class extends PageBase {
        ...
        for (let col of this.modEdits) {
            ...
            if (col.c_type === "datetime" || col.c_type === "date") {
                ...
                let options = {
                    ...
                    accept: {
                        title: '文件',
                        extensions: this.mod.parmsUrl.c_type == 0 ? '*' : 'jpg,png,jpeg,gif,bmp,doc,docx,pdf,xls',
                        mimeTypes: this.mod.parmsUrl.c_type == 0 ? '.*' : '.jpg,.png,.jpeg,.gif,.bmp,.doc,.docx,.xls,.pdf'
                    }
                };
        ...
        if (act.id > 0) {
            ...
            if (linkRec.c_status != act.c_domain_st) {
                ...
                if (this.mod.c_modulename != 'Appr' && think.isEmpty(this.mod.parmsUrl.linkModulename)) {
                    console.log("333333333334444444444444?")
    ...
    async htmlGetView(isPrintStyle) {
        ...
        for (let col of this.modCols) {
            ...
            if (isPrintStyle) {
                ...
                html.push(`<td> <label class="control-label x120">${col.c_name}: </label>`);
            }
    ...
    async htmlGetListTimeline() {
        ...
        for (let row of this.list.data) {
            ...
            console.log("----------------user----------------");
            console.log(row.c_user);
            html.push('<li>');
            html.push(`<div class="time">${row.c_time}</div>`);
            html.push(`<div class="version">${row.c_status}</div>`);
            html.push(`<div class="number"></div>`);
            html.push(`<div class="content"><pre><p>${think.isEmpty(row.c_desc) ? '':'批注说明： '+row.c_desc}</p><p>${think.isEmpty(row.c_memo) ? '':'其他备注： ' +row.c_memo}</p><p>${think.isEmpty(row.c_user) ? '':'审批人： ' +row.c_user}</p></pre></div>`);
            html.push('</li>');
        }
