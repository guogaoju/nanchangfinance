...
module.exports = class extends docuRec {
    ...
    async getStockByID(good_id,docu_id){

        let stock_id = await cmpage.service('docu/docu_list').getStock(docu_id);
        ...
        let qty = await this.query(`select sum(c_qty) as qty from nanchang_docu.vw_stock_goods where c_goods='${good_id}' and c_stock='${stock_id}'`);
        if(qty.length == 0){
            return -1;
        }else{
            return qty[0].qty;
