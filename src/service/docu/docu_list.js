...
module.exports = class extends CMPage {
    ...
    async getQueryWhere() {
        ...
        if (this.docuType.id == cmpage.enumDocuType.DocuInvalid || this.docuType.id == cmpage.enumDocuType.Order || this.docuType.id == cmpage.enumDocuType.OrderApply) {
            where += ` and c_group in(${this.mod.user.groups})`;
        } else if (this.docuType.id == cmpage.enumDocuType.DocuInvalid) {
            where += ` and c_group in(${this.mod.user.groups})`;
        } else {
            where += ` and c_type=${this.docuType.id} and c_group in(${this.mod.user.groups})`;
        }
    ...
    async getDataRecord() {
        ...
        console.log("------------------------------");
        console.log(this.mod.editID);
        console.log(md);
        console.log("------------------------------");
        if(this.mod.editID>0 ){
            if ( md.c_status != 1211 && md.c_status != 0 ) {
    ...
    async getStock(id){
        let stock = await this.query(`select c_stock from nanchang_docu.t_docu where c_id = ${id}`);
        return stock[0].c_stock;
    }
