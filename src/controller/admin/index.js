...
module.exports = class extends Base {
    ...
    async indexAction() {
        ...
        let titleMd = await cmpage.service('admin/code').getCodeById(1409); //系统版本
        let vb = {
            groupName: user.groupName || "",
            version: codeMd.c_desc,
            title: titleMd.c_desc
        };
        ...
        //取主菜单
        let menuHtml = [];
        if (think.env === 'development' && user.c_login_name=='admin') {
            menuHtml.push(`<li>
        ...
        // console.log("----------------------------------");
        if(cmpage.checkMobile(this.userAgent)) {
    ...
    async loginAction() {
        ...
        if (this.isGet) {
            ...
            if (!think.isEmpty(user)) {
                if (user.c_status != 1) {
                    ...
                    this.assign('vb', vb);
                    if(cmpage.checkMobile(this.userAgent)) {
                        return this.redirect('/cmpage/hh/admin_login?msg='+vb.msg);
                    } else {
                        return this.display();
                    }
                }
                ...
                this.assign('vb', vb);
                if(cmpage.checkMobile(this.userAgent)) {
                    return this.redirect('/cmpage/hh/admin_login?msg='+encodeURI(encodeURI(vb.msg)));
                } else {
                    return this.display();
                }
            }
        ...
        this.assign('vb', vb);
        if(cmpage.checkMobile(this.userAgent)) {
            return this.redirect('/cmpage/hh/admin_login');
        } else {
            return this.display();
        }
    }
    ...
    async homeAction() {
        let vb = {};
        vb.suppliercnt = await cmpage.service('admin/scf').getSCFSupplierCnt();
        vb.recievecnt = await cmpage.service('admin/scf').getSCFRecieveCnt();
        vb.buildcnt = await cmpage.service('admin/scf').getSCFBuildCnt();
        vb.moneycnt = await cmpage.service('admin/scf').getSCFMoney();
        if(vb.moneycnt>10000){
            vb.moneycnt = Math.round(vb.moneycnt/10000)+'万';
        }
        console.log("-------------vb---------------------");
        cmpage.warn(vb);
        this.assign('vb', vb);
    ...
    async startRankAction(){
        let rank = await cmpage.service('admin/crontab_exe').startrank(1);
    }
