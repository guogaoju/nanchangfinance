...
module.exports = class extends Base {
    ...
    async excel_exportAction() {
        ...
        let excel = await cmpage.service('cmpage/page_excel').excelExport(pageModel.list, pageModel.modCols);
        //cmpage.debug(vb.listHtml);
    ...
    async edit_msAction() {
        ...
        let editHtml = await pageModel.htmlGetEdit();
        let tabsHtml = "";
        if(think.isEmpty(this.get("c_no"))){
            tabsHtml = pageModel.htmlGetTabs();
        }else{
            tabsHtml = pageModel.htmlGetTabs(this.get("c_no"));
        }
    ...
    async view_msAction() {
        ...
        console.log("--------------------------------");
        console.log(pageModel.mod.parmsUrl);
        console.log("--------------------------------");
...
console.log("-----------------erer--------------");
    ...
    async uploadMobileAction() {

        const fs = require('fs');
        const path = require('path');

        const file = this.post('file');
        const link_type = this.post('c_link_type');

        //console.log(file);
        console.log(link_type);

        var base64 = file.replace(/^data:image\/\w+;base64,/, ""); //去掉图片base64码前面部分data:image/png;base64
        ...
        //console.log(file);
        console.log("----------------upload_fileAction-------------");
        var file_name = new Date().getTime()+Math.floor((Math.random()*1000))+'.jpg';
        console.log(file_name);
        let fileName = `/static/upfiles/${link_type}/${cmpage.datetime().substring(0,4)}/`;
        console.log(fileName);
        

        let saveDir = path.join(think.ROOT_PATH, `www${fileName}`);
        think.mkdir(saveDir);

        let saveFile = saveDir+file_name;
        ...
        fs.writeFile(saveFile, dataBuffer, (err) => {
            if (err) {
                console.log(err);
                throw err;
            }
    ...
    async upload_fileAction() {
        ...
        // console.log(file);
        let fileName = `/static/upfiles/import/${file.name}`;
